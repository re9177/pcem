...
void lpt1_device_init() {
        ...
        while (lpt_devices[c] != NULL && strcmp(lpt_devices[c]->internal_name, lpt1_device_name) &&
               strlen(lpt_devices[c]->internal_name) != 0)
                c++;
...
void lpt2_remove_ams() { io_removehandler(0x0379, 0x0002, lpt2_read, NULL, NULL, lpt2_write, NULL, NULL, NULL); }
